    ...
    def __init__(self, scope, trainer, x_dim_p, x_dim_v, n_actions, num_layers, layer_size, regu_scalar):
        ...
        with tf.compat.v1.variable_scope(self.scope):
            ...
            if self.scope != 'global':
                ...
                #Loss functions
                self.regu_loss_policy = tf.add_n(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES, scope = self.scope + '/policy'))
                self.entropy = - tf.reduce_sum(self.policy * tf.compat.v1.log(self.policy + 1e-6)) # 1e-6 is for preventing NaN
                ...
                self.regu_loss_value = tf.add_n(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES, scope = self.scope + '/value'))
                ...
                local_vars_policy = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope = self.scope + '/policy')
                ...
                local_vars_value = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope = self.scope + '/value')
                ...
                global_vars_policy = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, 'global' + '/policy')
                self.apply_grads_policy = self.trainer.apply_gradients(zip(grads_policy, global_vars_policy))
                
                global_vars_value = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, 'global' + '/value')
