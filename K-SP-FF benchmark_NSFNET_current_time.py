...
def release(slot_map, request_set, slot_map_t, current_time):  # update slotmap to release FS' occupied by expired requests
    if request_set:
        # update slot_map_t
        for ii in range(LINK_NUM):
            for jj in range(SLOT_TOTAL):
                if 0 < slot_map_t[ii][jj] < current_time:
                    slot_map_t[ii][jj] = 0
                # N.B. different results in you use current time vs time to!!!!


        del_id = []
        for rr in request_set:
            if request_set[rr][3] <= current_time:
...
if __name__ == "__main__":
    ...
    for ex in range(10):
        ...
        while req_id < num_req_measure + warmup:

            arrival_time = np.random.exponential(lambda_intervals)
            current_time += arrival_time

            (slot_map, request_set, slot_map_t) = release(slot_map, request_set, slot_map_t, current_time)
            ...
            if True:  # If is used just for the sake of convenience...
                ...
                #holding_time = 0
                #while holding_time == 0 or holding_time >= service_time * 2:
                holding_time = np.random.exponential(service_time)
